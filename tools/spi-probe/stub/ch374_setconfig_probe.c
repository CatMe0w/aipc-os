...
static uint32_t control_status_in(uint32_t w0, uint32_t w1, uint32_t base)
{
    ...
    for (uint32_t retry = 0; retry < 20u; retry++) {
        ret = issue_token(CH374_PID_IN, 1u);
        OUT[base + 4u] = ret;
        OUT[base + 5u] = g_irq;
...
void stub_main(void)
{
    uint32_t setaddr_ret;
    uint32_t setcfg_ret = 0;
    ...
    setaddr_ret = control_status_in(0x00020500u, 0u, 16u);
    delay_ms(2u);
    reg_write(8u, 2u);

    if (setaddr_ret == CH374_OK)
        setcfg_ret = control_status_in(0x00010900u, 0u, 32u);
