...
static void store_byte(uint32_t *dst, uint32_t word)
{
    *dst = word & 0xFFu;
}
...
static uint32_t set_address2(void)
{
    uint32_t ret;

    set_addr_data8(0x00020500u, 0u);
    reg_write(11u, 8u);
    delay_short(200u);
