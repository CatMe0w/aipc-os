...
static uint32_t get_config9(void)
{
    uint32_t requested = 9u;
    uint32_t got = 0;
    uint32_t toggle = 1u;
    uint32_t *desc = (uint32_t *)&OUT[48];
    uint32_t ret;

    for (uint32_t i = 0; i < 4u; i++)
        desc[i] = 0;

    set_addr_data8(0x02000680u, 0x00090000u);
    reg_write(11u, 8u);
    delay_short(200u);
    ...
    for (uint32_t tries = 0; tries < 2u && requested != 0; tries++) {
        uint32_t chunk;
