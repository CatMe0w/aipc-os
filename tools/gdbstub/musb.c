 ...
 *
 * Data does not move through the FIFO ports. Payloads are staged in L2 SRAM
 * windows; the FIFO port is poked once per word purely to advance the hardware
 * pointer. Transmit sequence: FORBID_WRITE, fill L2 while poking FIFO,
 * TX_COUNT, PRE_READ, CSR ready. Deviating from that order silently kills the
 * endpoint.
 */
...
/* Echoed back in GET_LINE_CODING; nothing here reaches real hardware. */
...
/* How the device would look at the other speed; mandatory for HS capable. */
...
/* Template; descriptor type and bulk packet sizes are patched per request. */
...
/* HSMODE survives a soft reset, so a stub started by another stub reads the
 * speed of the session it inherited. */
...
void musb_init(void)
{
    ...
    /* SOFTCONN is not implemented; the pull-up survives everything, so there
     * is no way to signal a disconnect from software. */
    ...
    /* MODE_FORCE_FS sizes the data path; HSENAB arms the chirp. Both are
     * needed, and HSENAB must be set before the host's reset arrives. */
...
{
    /* The reset interrupt fires after the chirp, so POWER already has the
     * negotiated speed. */
    ...
    /* Always 512; the L2 staging path only works with this value. Full speed
     * is held to 64 by chunking in software. */
    configure_endpoint(EP_BULK_IN, 512, 1);
    configure_endpoint(EP_BULK_OUT, 512, 0);

    /* EP_NOTIFY left untouched; it NAKs, which is correct for CDC idle. */
    REG8(USB_INDEX) = 0;
...
static void ep0_send_chunk(void)
{
    ...
    if (last) {
        /* Final packet: byte-wide pokes so count matches the packet length. */
        for (i = 0; i < count; i++)
            REG8(USB_FIFO_EP0) = 0;
    ...
    /* TxPktRdy then DataEnd as two writes; one combined write doesn't work. */
...
static void handle_setup(void)
{
    ...
    case 5:                                     /* SET_ADDRESS */
        /* Spin for EP0 TX completion before writing FADDR. */
        ep0_status();
...
void musb_bulk_send(const uint8_t *data, uint32_t len)
{
    while (len) {
        ...
        /* A final full packet would sit in the host's URB; shorten it. */
        ...
        /* Wire length follows the FIFO pointer, not TX_COUNT; a non-word-
         * aligned length must poke the FIFO a byte at a time. */
        if (count & 3u) {
            for (i = 0; i < count; i++)
                REG8(USB_FIFO_EP2) = 0;
            for (i = 0; i < count; i += 4)
...
/* Words, not bytes: avoids a second pass with the MMU off. */
...
static void handle_bulk_out(void)
{
    ...
    /* Re-read; reusing a snapshot loses packets. */
    REG8(USB_INDEX) = EP_BULK_OUT;
...
void musb_poll(void)
{
    /* Sample once; re-reading mid-dispatch risks losing a bit. */
    ...
    if (intrtx & 0x01u) {
        ...
        if (csr & CSR0_SETUPEND) {
            g_ep0_out_remaining = 0;            /* host abandoned the transfer */
...
void musb_report(void)
{
    ...
    /* Interrupt status registers clear on read; don't touch them here. */
