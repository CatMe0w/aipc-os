...
/* Console output is hex encoded, and the 'O' takes a byte of the body. */
#define MONITOR_CHUNK ((PACKET_SIZE - 8u) / 2u)

#define TRACE_CMD_LIMIT 60u

/* .data: start.S fills this before .bss is zeroed. */
...
/* The one planted to land a single step, removed as soon as it fires. */
...
/* '+' acknowledges the command; only the first packet of a multi-packet reply
 * carries one. */
...
/* Target reads stay byte wide so hardware registers see byte-lane accesses. */
...
/* Long text, sent as console output packets then closed with OK. */
...
/* Decimal, and without division: there is no libgcc here. */
...
/* `monitor md32 0x<addr> [<words>]`. Word-wide complement to byte-wide `m`. */
...
/* qRcmd; emits its own packets, so it runs before dispatch() opens one. */
...
/* First action from vCont; one thread, so first action applies. */
...
static uint32_t bp_clear(uint32_t addr)
{
    ...
    return 1;               /* removing one that is not set is not an error */
}

/* Software breakpoints only; hardware ones are on the JTAG scan chain. */
...
/* Binary write (`X`). Word stores when aligned and no escapes; MMU off means
 * each byte store is its own bus transaction. */
...
static void do_step(void)
{
    ...
    /* Branch to self: nowhere to stop, report immediately. */
    if (next == g_regs.pc) {
        out_begin();
        out_ch('S');
...
static void dispatch(void)
{
    ...
    case 'k':
        ...
        break;
    default:
        /* Empty reply for unsupported ops.
         * Covers v, z, Z, T and everythingelse. */
        break;
    }
...
/* Framing only; the body is copied in runs by feed_body(). */
static void feed_byte(uint8_t c)
{
    ...
    case ST_IDLE:
        ...
        /* Ack/nak/break ignored; no retransmit, no interrupt target. */
        break;
...
/* Copy the run up to '#', checksumming as it goes. Returns bytes consumed. */
static uint32_t feed_body(const uint8_t *data, uint32_t len)
{
    ...
    while (i < len) {
        ...
        if (c == '#')
            break;
        if (n >= CMD_SIZE) {
            g_state = ST_IDLE;              /* oversized, drop it */
            break;
        }
        g_cmd[n++] = c;
        sum += c;
