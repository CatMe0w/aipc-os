...
/* Word access into byte arrays without breaking strict aliasing. */
...
/* Vendor block: data moves through L2, these gate and trigger it. */
...
/* L2 windows assigned to USB (L2CTR_ASSIGN_REG1 low 6 bits = 8). */
...
/* POWER, peripheral mode. SOFTCONN (bit 6) is not implemented on this part. */
#define POWER_HSMODE        (1u << 4)   /* read-only; set if chirp negotiated HS */
...
#define EP_NOTIFY           1u      /* CDC notification, declared and unused */
...
/* Must hold the largest descriptor rounded up to a whole number of packets. */
#define EP0_BUF_SIZE        128u

/* Software must chunk at the negotiated size; TXMAXP/RXMAXP stay at 512. */
...
/* Endpoint counters and live register state, for the post-mortem trace. */
