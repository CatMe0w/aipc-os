...
11. Rebuild ROM modules into IDA-oriented PE files under `NK.ecec_NN.modules/`. `ImageBase` is set to `module.load_pointer`; section bytes are read from each section's own `o32_rom.data_pointer`; in-image absolute pointers are rebased from the compact descriptor's `image_base` to `load_pointer`; export and import directories are synthesized from `e32_rom` unit entries.
