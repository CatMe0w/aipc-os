...
/* Write the divider only if the controller is unconfigured. The v1.88 bootrom
 * sets a different one, and an overwrite would change the baud rate. */
void uart_init(void)
{
...
void uart_putc(char c)
{
    ...
    /* Bounded, so that a broken UART cannot hang the system. */
    for (uint32_t i = 0; i < UART_TX_LIMIT && (UART_COUNT & 0x1FFFu) != 0u; ++i)
        ;
}

/* Both NAND and MMC stall if bits 29:28 are clear. The bootrom sets them on a
 * real boot, but in USB boot mode nothing sets them. */
void l2_init(void)
{
...
/* Reload SP, because our call frames moved it, and unmask IRQ and FIQ. This is
 * the state that the OEM nboot leaves for EBOOT. */
