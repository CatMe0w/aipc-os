/*
 * Where execution goes after the instruction at g_regs.pc. ARM state only.
 * A Thumb pc is undecidable here.
 */
...
/* r15 reads as pc+8. sp and lr are outside the array. */
...
/* A shift amount of zero means 32 for LSR and ASR, and RRX for ROR. */
...
uint32_t arm_next_pc(uint32_t *next)
{
    ...
    /* 0xF is the unconditional space. Only BLX(imm) branches there, and it
     * enters Thumb. */
    if (cond == 0xF)
        return 0;
    ...
    if ((insn & 0x0E000000u) == 0x0A000000u) {
        int32_t off = (int32_t)(insn << 8) >> 6;     /* sign extend, then x4 */
    ...
    if ((insn & 0x0E100000u) == 0x08100000u && (insn & (1u << 15))) {
        ...
        /* pc has the highest register number, thus it is always at the top. */
        if (up)
    ...
    /* Data processing with pc as the destination. The compares write nothing,
     * so this skips them. */
