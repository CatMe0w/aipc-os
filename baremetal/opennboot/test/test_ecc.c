/* Six chunks with a single bit error, from the v1.58.2 device. Hardware
 * confirms four of them. The other two are predictions. */
...
/* An independent inverse, written from the closed form, not from ecc.c. */
...
static void test_apply_flips_one_bit(void)
{
    for (size_t k = 0; k < sizeof(fixtures) / sizeof(fixtures[0]); k++) {
        ...
        data[fixtures[k].byte] ^= fixtures[k].mask;   /* inject the bit flip */
...
static void test_tag_region(void)
{
    ...
    /* One byte past the tag scratch must give a refusal, not a write. */
