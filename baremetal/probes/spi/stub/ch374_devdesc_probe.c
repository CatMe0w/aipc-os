...
static void snapshot(uint32_t base)
{
    OUT[base + 0] = reg_read(2u);
    OUT[base + 1] = reg_read(3u);
    OUT[base + 2] = reg_read(5u);
    OUT[base + 3] = reg_read(6u);
    OUT[base + 4] = reg_read(7u);
    OUT[base + 5] = reg_read(8u);
    OUT[base + 6] = reg_read(9u);
    OUT[base + 7] = reg_read(10u);
    OUT[base + 8] = reg_read(11u);
    OUT[base + 9] = reg_read(14u);
}
...
static uint32_t get_device_desc(void)
{
    uint32_t requested = 18u;
    uint32_t got = 0;
    uint32_t toggle = 1u;
    ...
    set_addr_data8(0x01000680u, 0x00120000u);
    reg_write(11u, 8u);
    delay_short(200u);
    ...
    for (uint32_t tries = 0; tries < 4u && requested != 0; tries++) {
        uint32_t chunk;

        for (uint32_t retry = 0; retry < 20u; retry++) {
            ret = issue_token(CH374_PID_IN, toggle);
            OUT[40 + tries * 4u + 0u] = ret;
            OUT[40 + tries * 4u + 1u] = g_irq;
            OUT[40 + tries * 4u + 2u] = g_result;
            OUT[40 + tries * 4u + 3u] = retry;
    ...
    reg_write(11u, 0u);
    OUT[36] = issue_token(CH374_PID_OUT, 1u);
    OUT[37] = g_irq;
    OUT[38] = g_result;
    return got;
}
