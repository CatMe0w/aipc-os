...
2. SYSCTRL+0x78: set bits [23:22, 17, 9] (0xC70200) - enables NF-related
   clock and I/O paths.
3. L2CTR_ASSIGN_REG1 (0x2002C090): clear bits [11:9] - unbind any previous
   L2 buffer assignment for the NF path.
4. L2CTR_COMBUF_CFG (0x2002C080): set bit 16 (enable), then set bit 24
   (flush/reset the common buffer).
5. L2CTR_DMAFRAC (0x2002C08C): set bits [29:28].
6. NF timing register 0 (0x2002A05C): write default value 1006545
   (0x0F5B51).

## Probe Procedure

1. Iterate through 8 probe parameter sets (`spi_probe_params[0..7]`). Despite
...
3. Write dummy cycle words: `0x62` repeated `dummy_count` times.
...
7. Copies `byte_count / 4` words from L2BUF_00 base (0x48000000) to `dst`,
   then flushes by setting L2CTR_COMBUF_CFG bit 24.
...
Each of the 8 probe parameter sets (`spi_probe_param_ex_t`) contains:

| Field           | Size | Description                                                                                 |
| --------------- | ---- | ------------------------------------------------------------------------------------------- |
| counts          | 4B   | Packed: byte0 = chunks_per_page, byte1 = page_count, byte2 = cmd_count, byte3 = dummy_count |
| command         | 4B   | cmd1 (byte), addr_byte_count (byte), cmd2 (byte), padding                                   |
| timing_cfg0     | 4B   | NF timing register 0 override (0 = keep default)                                            |
| timing_cfg1     | 4B   | NF timing register 1 override (0 = keep default)                                            |
| pre_delay_ticks | 2B   | Delay in ticks before data read                                                             |
| extra_cfg       | 2B   | Additional config [unverified]                                                              |

The 8 parameter sets cover different NAND flash configurations (varying
page sizes, command sequences, and timing requirements) to support
auto-detection of the connected flash device.
