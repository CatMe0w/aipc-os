...
- `GPIO69` carries the net `LCD_D6`, and that net never reaches the panel. It drives the enable of the two speaker amplifiers, active high. `SYSCTRL+0x78` bit 27 muxes `GPIO69` and `GPIO70` to `LCD_Data16` and `LCD_Data17`, which a panel of sixteen data lines does not use, thus the bit has to stay clear. With it set, the pixel stream drives the amplifier enable. See [docs/soc/audio.md](../soc/audio.md).
