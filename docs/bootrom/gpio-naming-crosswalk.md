...
The table below is flattened one row per GPIO or DGPIO name visible on the AK7802 QFP216 schematic symbol.
...
- `GPIO1`..`GPIO4` are our internal names for four 32-bit SYSCTRL register windows. They are not the names used by the schematic.
- `GPIO2[27]` and `GPIO2[28]` do not have named package pins in the QFP216 schematic pin tables; this matches the bootrom diagnostic mask `0xE7FFFFFF`.
- The ordering of `GPIO4[10]` and `GPIO4[11]` against `DGPIO19` and `DGPIO28` is still unresolved.
