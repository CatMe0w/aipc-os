...
/* Sequencer bank 1; bank 0 is the bootrom's. */
...
/* Fixed for this part. Only pages_per_block is discovered at runtime. */
 ...
 * n is a whole number of words and never exceeds one L2 buffer. */
    ...
    *fifo   = 0x201u;                               /* wait ready, end */
    ...
    for (uint32_t c = 0; c < CHUNKS; ++c) {
        ...
        case ECC_CORRECTABLE: {
            /* Engine does not correct in place; tag corrections go to scratch. */
...
/* A wrong pages_per_block lands on a page that does not carry the IPL magic,
 * so a wrong guess will not happen. */
...
/* Bad blocks are skipped whole, matching the OEM writer's rule. Unlike the stock
 * nboot, a read error stops instead of skipping ahead two blocks. */
