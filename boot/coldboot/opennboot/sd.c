...
/* Buffer 4 for MCI, buffer 5 for NFC -- no contention on fallback. */
 ...
 * but they sit behind a layer with its own timeout and this does not. */
...
/* Captured at sd_init() entry, restored by sd_release_pins(). start.S must
 * clear 0x0C before we get here, or the fallback would restore the bootrom's
 * clock gating. */
static struct {
...
/* Writing the divider field implicitly clears CLK_EN, so we restore it. */
...
/* The first transfer after init reliably times out; the OEM Linux driver
 * handles this via block-layer retries. One retry has always been enough. */
